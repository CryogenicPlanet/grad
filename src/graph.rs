...
pub mod tensor {
    ...
    fn trace<T: TensorType>(
        root: Tensor<T>,
    ) -> (HashSet<Tensor<T>>, HashSet<(Tensor<T>, Tensor<T>)>) {
        let mut nodes: HashSet<Tensor<T>> = HashSet::new();
        let mut edges: HashSet<(Tensor<T>, Tensor<T>)> = HashSet::new();

        fn build<T: TensorType>(
            v: &Tensor<T>,
            nodes: &mut HashSet<Tensor<T>>,
            edges: &mut HashSet<(Tensor<T>, Tensor<T>)>,
        ) {
            if !nodes.contains(v) {
                nodes.insert(v.clone());
                if let Some(children) = &v.get_children() {
    ...
    pub fn draw_dots<T: TensorType>(root: Tensor<T>) {
        let mut deps = Graph::<&str, &str>::new();
        ...
        let mut node_lookup_input: HashMap<Tensor<T>, NodeIndex> = HashMap::new();
        let mut node_lookup_output: HashMap<Tensor<T>, NodeIndex> = HashMap::new();
